...
public class SQLDisponibilidad {
    ...
    /**
     * Crea y ejecuta la sentencia SQL para adicionar un DISPONIBILIDAD a la base de
     * datos de Alohandes
     * 
     ...
     */
    public long adicionarDisponibilidad(PersistenceManager pm, Timestamp fecha, long idHabitacion, long idReserva,
            String disponible) {
        Query q = pm.newQuery(SQL, "INSERT INTO " + pp.darTablaDisponibilidad()
                + "(fecha, idHabitacion, idReserva,disponible) values (?, ?,?,?)");
    ...
    public long eliminarDisponibilidad(PersistenceManager pm, Timestamp fecha, long idHabitacion) {
        Query q = pm.newQuery(SQL,
                "DELETE FROM " + pp.darTablaDisponibilidad() + " WHERE fecha = ? AND idHabitacion = ?");
